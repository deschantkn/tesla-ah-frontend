...
describe('SEARCH reducer tests...', () => {
  ...
  test('When Search is done', () => {
    ...
    const state = searchReducer(initialState, ({ type: SEARCH_DONE, payload: data }));
    expect(state).toEqual(expectedState);
  });
  test('Reinitialize the reducer', () => {
    const state = searchReducer(initialState, { type: SEARCH_CLEAR, payload: null });
    expect(state).toEqual(initialState);
  });
  test('In case the search failed', () => {
    ...
    const state = searchReducer(initialState, { type: SEARCH_FAILED, payload: 'Request failed' });
    expect(state).toEqual(expectedState);
  });
  test('The search request is pending', () => {
    ...
    const state = searchReducer(initialState, { type: SEARCH_PENDING, payload: null });
    expect(state).toEqual(expectedState);
  });
});
